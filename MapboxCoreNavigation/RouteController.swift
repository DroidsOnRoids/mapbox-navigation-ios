...
extension RouteController: CLLocationManagerDelegate {
    ...
    @objc func interpolateLocation() {
        ...
        guard let interpolatedCoordinate = lineString.coordinateFromStart(distance: routeProgress.distanceTraveled+distance) else {
            return
        }

        var course = location.course
        if let upcomingCoordinate = lineString.coordinateFromStart(distance: routeProgress.distanceTraveled+(distance*2)) {
    ...
    @objc public func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        let lineString = LineString(routeProgress.currentLegProgress.currentStep.coordinates!)
        if let closestCoordinate = lineString.closestCoordinate(to: location.coordinate),
    ...
    func updateDistanceToIntersection(from location: CLLocation) {
        ...
        if let upcomingIntersection = routeProgress.currentLegProgress.currentStepProgress.upcomingIntersection {
            routeProgress.currentLegProgress.currentStepProgress.userDistanceToUpcomingIntersection = LineString(currentStepProgress.step.coordinates!).distance(from: location.coordinate, to: upcomingIntersection.location)
        }
    ...
    func updateIntersectionDistances() {
        if let coordinates = routeProgress.currentLegProgress.currentStep.coordinates, let intersections = routeProgress.currentLegProgress.currentStep.intersections {
            let lineString = LineString(coordinates)
            let distances: [CLLocationDistance] = intersections.compactMap { lineString.distance(from: coordinates.first, to: $0.location) }
