...
extension CLLocation {
    ...
    func interpolatedCourse(along coordinates: [CLLocationCoordinate2D]) -> CLLocationDirection? {
        let nearByLineString = LineString(coordinates)
        
        guard let closest = nearByLineString.closestCoordinate(to: coordinate) else { return nil }
        
        let slicedLineBehind = LineString(coordinates.reversed()).sliced(from: closest.coordinate, to: coordinates.reversed().last)
        let slicedLineInFront = nearByLineString.sliced(from: closest.coordinate, to: coordinates.last)
        ...
        guard let pointBehindClosest = nearByLineString.closestCoordinate(to: pointBehind) else { return nil }
        guard let pointAhead = slicedLineInFront?.coordinateFromStart(distance: userDistanceBuffer) else { return nil }
        guard let pointAheadClosest = nearByLineString.closestCoordinate(to: pointAhead) else { return nil }
