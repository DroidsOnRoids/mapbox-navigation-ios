...
open class NavigationMapView: MGLMapView, UIGestureRecognizerDelegate {
    ...
    */
    ...
    @objc public func addArrow(route: Route, legIndex: Int, stepIndex: Int) {
        ...
        guard let coordinatesNegative = lineString.trimmed(from: maneuverCoordinate, distance: -shaftLength)?.coordinates,
            let coordinatesPositive = lineString.trimmed(from: maneuverCoordinate, distance: shaftLength)?.coordinates else { return }

        let shaftCoordinates = Array(coordinatesNegative.reversed() + coordinatesPositive.suffix(from: 1))
     ...
     */
    @objc public func setOverheadCameraView(from userLocation: CLLocation, along coordinates: [CLLocationCoordinate2D], for bounds: UIEdgeInsets) {
        isAnimatingToOverheadMode = true
        guard let slicedLine = LineString(coordinates).sliced(from: userLocation.coordinate)?.coordinates else { return }
